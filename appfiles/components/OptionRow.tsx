...
export function OptionRow({ o }: { o: OptionPosition }) {
  const [open, setOpen] = useState(false);
  const toggle = () => setOpen((v) => !v);
  const dte = daysToExpiry(o.expiration);
  const pnl = optionPnl(o);

  // CSPs render as a compact table row; indicators move into the expanded detail.
  if (o.kind === "csp") {
    const coll = cspCollateral(o);
    const pnlPct = optionPnlPct(o);
    const toStrike = o.underlyingPrice && o.underlyingPrice > 0 ? (o.underlyingPrice - o.strike) / o.underlyingPrice : null;
    ...
    return (
      <div>
        <button onClick={toggle} className={`${CSP_COLS} w-full px-3 py-2.5 text-left text-[11px] active:bg-surface-2`}>
          ...
          <span className={`tabular text-right ${toStrike != null && toStrike < 0 ? "text-rose-400" : ""}`}>
  ...
  return (
    <div className="border-t border-border bg-surface-2/40 px-4 py-3">
      <div className="mb-2 flex flex-wrap items-center gap-1.5 text-sm font-medium">
        ...
        <Pill className="bg-sky-500/10 text-sky-300 ring-sky-500/20">short</Pill>
