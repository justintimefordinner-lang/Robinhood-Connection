...
// Robinhood's option instrument pages live at robinhood.com/options/{uuid}_{suffix} —
// `o.id` is the instrument UUID (comes straight through from Robinhood's own API via
// robinhood_client.py/export_to_app.py, never regenerated by this app). The suffix
// after the UUID is required, not optional routing fluff: puts get "S1", calls get
// "L1" (per confirmed observation — not just "short vs long", every put uses S1 and
// every call uses L1 regardless of whether the position itself is short or long).
function robinhoodOptionUrl(id: string, optionType: "put" | "call"): string {
  const suffix = optionType === "put" ? "S1" : "L1";
...
function CspDetail({ o }: { o: OptionPosition }) {
  ...
  return (
    <div className="border-t border-border bg-surface-2/40 px-4 py-3">
      ...
      <RobinhoodLink id={o.id} optionType={o.optionType} />
    </div>
  );
}
