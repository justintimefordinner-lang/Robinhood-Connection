...
// Robinhood's option instrument pages live at robinhood.com/options/{uuid} — `o.id`
// is that same instrument UUID (it comes straight through from Robinhood's own API
// via robinhood_client.py/export_to_app.py, never regenerated by this app). Robinhood's
// own links sometimes append a suffix like "_S1" (seen when navigating from an order
// confirmation); that appears to just be routing-state and the bare UUID resolves fine,
// but flagging it here in case it ever needs revisiting.
function robinhoodOptionUrl(id: string): string {
