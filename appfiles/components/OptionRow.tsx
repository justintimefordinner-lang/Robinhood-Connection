...
// Only a real Robinhood instrument id links anywhere — hand-entered positions and
// the example data carry ids of their own, so they get no button.
const ROBINHOOD_ID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
...
function RobinhoodLink({ id, optionType }: { id: string; optionType: "put" | "call" }) {
  if (!id || !ROBINHOOD_ID.test(id)) return null;
  return (
    <a
